...
class CartController extends Controller
{
    public function addToCart(Request $request)
    {
        ...
        if ($product->stock < $request->qty){
            $rr = [
                'cartError' => 'Oui',
                'cartErrorMessage' => 'Désolé '.$product->stock.' - '.'Produit non disponible'
            ];
            ...
            $cartShowTop = "<li class=\"cart-icon\">
                                    <a href=\"#\">
                                        ...
                                        <div class=\"cart-text hidden-sm hidden-xs\">Mon panier</div>
                                    </a>
                                    <div class=\"cart-dropdown header-link-dropdown\">
                                        <ul id=\"cartShow\" class=\"cart-list link-dropdown-list\">";

            $cartShowBottom = "</ul>
                                        <p class=\"cart-sub-totle\"> <span class=\"pull-left\">Panier Sous-total</span> <span class=\"pull-right\"><strong class=\"price-box\"> $subtotal </strong></span> </p>
                                        <div class=\"clearfix\"></div>
                                        <div class=\"mt-20\"> <a href=\"$cartRoute\" class=\"btn-color btn\">Panier</a> <a href=\"$CheckRoute\" class=\"btn-color btn right-side\">Commande</a> </div>
                                    </div>
                                </li>";
    ...
    public function singleAddToCart(Request $request)
    {
        ...
        $cartShowBottom = "</ul>
                                    <p class=\"cart-sub-totle\"> <span class=\"pull-left\">Sous total</span> <span class=\"pull-right\"><strong class=\"price-box\"> $subtotal </strong></span> </p>
                                    <div class=\"clearfix\"></div>
                                    <div class=\"mt-20\"> <a href=\"$cartRoute\" class=\"btn-color btn\">Panier</a> <a href=\"$CheckRoute\" class=\"btn-color btn right-side\">Commande</a> </div>
                                </div>
                            </li>";
    ...
    public function updateCartItem($rowId,$qty)
    {
        ...
        if ($product->stock < $qty){
            $rr = [
                'cartError' => 'Oui',
                'cartErrorMessage' => 'Sorry '.$product->stock.' - '.'Produit non disponible'
            ];
            return $result = json_encode($rr);
        }else{
    ...
    public function singleWishList(Request $request)
    {
        if (Auth::check()){
            ...
            if ($w == 0){
                ...
                $rr = [
                    'cartError' => 'no',
                    'cartErrorMessage' => 'Produit ajouté au panier'
                ];
                return $result = json_encode($rr);
            }else{
                $rr = [
                    'cartError' => 'exist',
                    'cartErrorMessage' => 'Produit déjà dans Wishlist'
                ];
                return $result = json_encode($rr);
            }
            ...
            $rr = [
                'cartError' => 'oui',
                'cartErrorMessage' => 'Svp! connectez-vous'
            ];
            return $result = json_encode($rr);
        }
    }
        ...
        if ($product->stock < $request->qty){
            ...
            foreach ($ww as $w){
                ...
                $middle .= "<tr>
                                ...
                                <td>
                                    <button data-id=\"$w->product_id\" id=\"wish$w->id\" class=\"btn btn-color\"><i class=\"fa fa-cart-plus\"></i> Ajouter au panier</button>
                                </td>
                            </tr>";
            }

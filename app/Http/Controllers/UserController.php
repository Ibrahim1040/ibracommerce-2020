...
class UserController extends Controller
{
    ...
    {
        $data['page_title'] = 'Changer profil';
    ...
    public function updatePassword(Request $request)
    {
        ...
        try {
            ...
            if(Hash::check($request->old_password, $c_password)){
                ...
                session()->flash('message', 'Le mot de passe actuel ne correspond pas');
    ...
    public function submitReview(Request $request)
    {
        ...
        if ($reviewC == 0){
            ...
            $in['user_id'] = $user_id;
            Review::create($in);
            session()->flash('message','Commentaire envoyé.');
            session()->flash('type','success');
            return redirect()->back();
        }else{
            session()->flash('message','Déjà noté.');
            session()->flash('type','warning');
            return redirect()->back();
        }
    }
    ...
    public function submitComment(Request $request)
    {
        ...
        $in['user_id'] = $user_id;
        Comment::create($in);
        session()->flash('message','Commentaire soumis.');
        session()->flash('type','success');
        return redirect()->back();
    ...
    public function getCheckOut()
    {
        ...
        $data['user'] = User::findOrFail(Auth::user()->id);
        $data['country'] = json_decode(file_get_contents(base_path('resources/utils/names.json'),true));
    ...
    public function submitDetails(Request $request)
    {
        ...
        session()->flash('message','Détail livraison enregisté.');
        session()->flash('type','success');
    ...
    public function orderSubmit(Request $request)
    {
        if (Cart::count() == 0){
           ...
           // $this->sendInvoice(Auth::user()->id,$r->id);
            session()->flash('message','Coordonnées enregistrées avec succès.');
            session()->flash('type','success');
    ...
    {
        $data['page_title'] = "Resumé paiement";
            ...
            if ($id == 3){
                ...
                try {
                    ...
                    if ($sendto!="") {
                        ...
                        session()->flash('message', "Quelques problèmes avec l'api");
                        Session::flash('type', 'warning');
                        return redirect()->back();
                    }
                } catch (\PDOException $e) {
    ...
    {
        $data['page_title'] = $orderNumber.' - Détail Commande';
    ...
    {
        $data['page_title'] = 'Wishlist Utilisateur';

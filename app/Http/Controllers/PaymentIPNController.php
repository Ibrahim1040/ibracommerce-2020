...
class PaymentIPNController extends Controller
{
    ...
    public function perfectIPN()
    {
        ...
        if($hash==$hash2){
            ...
            if($_POST['PAYEE_ACCOUNT']=="$pay->val1" && $unit=="USD" && $amo == $data->usd){
                ...
                session()->flash('message', 'Erreur de paiement');
        ...
        try{
            ...
            if ($charge['status'] == 'succeeded' ) {
               ...
               // $this->paymentConfirm($order->user_id,$order->total,$request->custom,"Stripe Card");
                ...
                session()->flash('message','Quelque chose ne va pas avec la carte.');
                session()->flash('type','warning');
                return redirect()->route('payment',$request->custom);
            }

        }catch (\Exception $e){
            session()->flash('message','Quelque chose ne va pas avec Stripe.');
            session()->flash('type','warning');
            return redirect()->route('payment',$request->custom);
        }
    }
    public function skrillIPN()
    {
        ...
        {
            session()->flash('message','Quelque chose ne va pas avec Skrill.');
            session()->flash('type','warning');
    ...
    public function payzaIPN()
    {
        ...
        if (strlen($response) > 0) {
            if (urldecode($response) == "INVALID TOKEN") {
                session()->flash('message','Jeton non valide dans Payza.');
                session()->flash('type','warning');
                return redirect()->route('home');
            } else {
                ...
                if ($transactionStatus == 'Success') {
                    ...
                    session()->flash('message','Transaction non terminée avec Payza.');
                    session()->flash('type','warning');
                    return redirect()->route('payment',$custom);
                }
            }
        } else {
            session()->flash('message','Quelque chose ne va pas avec Payza.');
            session()->flash('type','warning');
            return redirect()->route('home');
        }
    }

...
class BasicSettingController extends Controller
{
    ...
    public function postChangePass(Request $request)
    {
        ...
        try {
            ...
            if(Hash::check($request->current_password, $c_password)){
                ...
                session()->flash('message', 'Le mot de passe ne correspond pas.');
    ...
    protected function putBasicSetting(Request $request,$id)
    {
        ...
        $basic->fill($in)->save();
        session()->flash('message', 'Données sauvegardées avec succès.');
        Session::flash('type', 'success');
        Session::flash('title', 'Success');
        return redirect()->back();
    }
    ...
    public function updateProfile(Request $request)
    {
        ...
        $admin->fill($in)->save();
        session()->flash('message','Mise à jour du profil réussite.');
        session()->flash('title','Success');
        session()->flash('type','success');
        return redirect()->back();
    }
    ...
    {
        $data['page_title'] = "Gestion Email Template";
    ...
    public function updateEmailTemplate(Request $request)
    {
        ...
        $basic->save();
        session()->flash('message', 'Paramètre de messagerie mis à jour.');
        Session::flash('type', 'success');
        return redirect()->back();
    }
    ...
    public function getGoogleAnalytic()
{
    ...
    public function updateGoogleAnalytic(Request $request)
    {
        ...
        $in = Input::except('_method','_token');
        $basic->fill($in)->save();
        session()->flash('message', 'Mise à jour de Google Analytic.');
        Session::flash('type', 'success');
        return redirect()->back();
    }
    ...
    public function updateLiveChat(Request $request)
    {
        ...
        $in = Input::except('_method','_token');
        $basic->fill($in)->save();
        session()->flash('message', 'Scripts de chat mis à jour.');
        Session::flash('type', 'success');
        return redirect()->back();
    }
